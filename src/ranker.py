...
def evaluate(candidates, references):
    ...
    for i in precision_scores:
        ...
        result_dic["@{}".format(i)] = {"F1":np.mean(f1_scores[i]) , "P":np.mean(precision_scores[i]) , "R":np.mean(recall_scores[i])}
    print("#########################")
...
#                 file_name,
                ...
                alpha=1,
                processors=8,
                rank_type = 'both'):  #both, local, global
    ...
    def extract_summary(self,):
        ...
        with open(os.path.join(self.file_path, "predicted.txt"), 'w', encoding= "utf-8") as f:
        ...
        result_dic = evaluate(predicted_candidation_stem, self.keyphrases)

        result_dic['dataset'] = self.file_path
        result_dic['hyperparameters'] = {"beta": self.beta  ,"lambda1" :  self.lambda1 ,
         "lambda2" :self.lambda2,  "alpha": self.alpha}
        with open(os.path.join(self.file_path,"output_results_" +self.rank_type+".json"), "w") as outfile:
...
if __name__ == "__main__":
    ...
    ranker = DirectedCentralityRnak(data, model, 
    beta=0.2, lambda1=1, lambda2=0.9, alpha=1.5, processors=8 ,     rank_type= "global"
    )
